...
// Position update
__global__ void updatePosition(const real_d *force,real_d *position,const real_d* velocity, const real_d * mass,const real_d timestep ) {
...
__device__ void lenardJonesPotential(const real_d *relativeVector,real_d * forceVector ,const real_d sigma , const real_d epislon) {
    ...
    forceVector[0] = epislonConstant * pow(sigmaConstant,6.0) * (2.0 *(pow(sigmaConstant,6.0)) - 1.0 ) * relativeVector[0] ;
    forceVector[1] = epislonConstant * pow(sigmaConstant,6.0) * (2.0 *(pow(sigmaConstant,6.0)) - 1.0 ) * relativeVector[1] ;
    forceVector[2] = epislonConstant * pow(sigmaConstant,6.0) * (2.0 *(pow(sigmaConstant,6.0)) - 1.0 ) * relativeVector[2] ;
}

// Copying the forces
__global__ void copyForces(real_d * fold,real_d * fnew) {
